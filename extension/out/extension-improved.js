...
function activate(context) {
    ...
    const autoShowUI = config.get('autoShowUI', true);
    const uiPosition = config.get('uiPosition', 'center');
...
function showCommitverUI(context) {
    // Get configuration
    const config = vscode.workspace.getConfiguration('commitver');
    const uiPosition = config.get('uiPosition', 'center');
    ...
    const panel = vscode.window.createWebviewPanel('commitverUI', 'COMMITVER', viewColumn, {
        enableScripts: true,
...
function generateSecureSettingsHTML(webview) {
...
function saveConfigurationSafely(config) {
    const workspaceFolders = vscode.workspace.workspaceFolders;
    if (!workspaceFolders || workspaceFolders.length === 0) {
...
function loadConfigurationSafely(webview) {
    const config = loadConfigurationSafelyFromDisk();
    webview.postMessage({
        command: 'loadConfig',
        config: config
...
//# sourceMappingURL=extension-improved.js.map